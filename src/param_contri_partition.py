...
def main(config_path):
    ...
    with torch.no_grad():
        for agg_idx, checkpoint in enumerate(checkpoint_list):
            if agg_idx < len(checkpoint_list) - 1:
                ...
                params_agg_t_1 = (
                    f"{config['checkpoint']}/agg/{checkpoint_list[agg_idx + 1]}"
                )
                ...
                for client_id in range(config["num_client"]):
                    filename = f"{config['checkpoint']}/client-{client_id}/{checkpoint}"
                    ...
                    contribution_arr[client_id] += float(
                        torch.sign(params_delta * client_params).sum()
                    )
