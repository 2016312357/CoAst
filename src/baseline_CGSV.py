    ...
    with torch.no_grad():
        for agg_idx, checkpoint in enumerate(checkpoint_list):
            if agg_idx > 0:
                ...
                agg_params_t = torch.nn.utils.parameters_to_vector(rand_model.parameters())

                params_agg_t_1 = f"{config['checkpoint']}/agg/{checkpoint_list[agg_idx-1]}"
                rand_model.load_state_dict(torch.load(params_agg_t_1))
                agg_params_t_1 = torch.nn.utils.parameters_to_vector(rand_model.parameters())

                update_params_agg = agg_params_t - agg_params_t_1
                ...
                for client_id in range(config["num_client"]):
                    ...
                    # filename_1 = f"{config['checkpoint']}/client-{client_id}/{checkpoint_list[agg_idx-1]}"
                    # net_1 = torch.load(filename_1).to(device)
                    # client_params_1 = torch.nn.utils.parameters_to_vector(net_1.parameters())

                    update_params_client = client_params - agg_params_t_1

                    cos_sim = F.cosine_similarity(update_params_agg, update_params_client, dim=0)
                    tmp_scores.append(float(cos_sim))
